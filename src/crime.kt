...
fun main() {
    ...
    val crimesPerDay = crimes.size / crimes.groupBy { it.date }.count()
    println("Crimes per day on average: $crimesPerDay")

    val highestCrimeDate = crimes.groupBy { it.date }.maxBy { it.value.size }!!.key
    val maxCrimes = crimes.groupBy { it.date }.maxBy { it.value.size }!!.value.count()
    println("$highestCrimeDate had the most reported crimes, with $maxCrimes reported.")

    val highestCrimeDistrict = crimes.groupBy { it.district }.maxBy { it.value.size }!!.key
    println("District $highestCrimeDistrict had the most crime reported.")

    val lowestCrimeDistrict = crimes.groupBy { it.district }.minBy { it.value.size }!!.key
    println("District $lowestCrimeDistrict had the least crime reported.")

    val distinctCrimes = crimes.groupBy { it.description }.keys.count()
    println("There were $distinctCrimes distinct types of crime reported.")

    val mostFrequentCrime = crimes.groupBy { it.description }.maxBy { it.value.size }!!.key
